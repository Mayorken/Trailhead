...
> **Status:** MVP. Builds and typechecks; the read path is verified end-to-end against a local
> node. Write actions (deposit/withdraw/follow) require an injected wallet on the target chain.
...
| `VITE_REGISTRY_ADDRESS` | Deployed `StrategyRegistry` |
| `VITE_VAULT_ADDRESS` | Deployed `FollowerVault` |
...
- Not yet pointed at a public testnet deployment (contracts aren't on Fuji yet).
